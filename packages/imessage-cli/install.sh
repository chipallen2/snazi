#!/usr/bin/env bash
#
# soup-nazi iMessage CLI installer (on-demand, NO daemon, NO launchd).
# Builds the CLI and scaffolds ~/.soup-nazi/config.json.
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_DIR="$HOME/.soup-nazi"
...
JSON
  ...
  echo "    Wrote template config. EDIT IT with your real apiUrl + apiKey."
else
...
NEXT STEPS
  1. Edit $CONFIG_PATH
       - apiUrl : your Vercel deployment URL
  ...
  3. (optional) Make 'soup-nazi' available on PATH:
